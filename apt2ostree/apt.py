...
make_dpkg_info = Rule(
    "make_dpkg_info", """\
        ...
        ostree --repo=$ostree_repo checkout --repo=$ostree_repo -UH "$ref_control" "$$tmpdir/control";
        ...
        ostree --repo=$ostree_repo ls -R $ref_data --nul-filenames-only
        | tr '\\0' '\\n' 
        ...
        cd -;
        ostree --repo=$ostree_repo commit -b "$ref_info" --tree=dir=$$tmpdir/out
            --no-bindings --orphan --timestamp=0 --owner-uid=0 --owner-gid=0
            --no-xattrs;
        overwrite_if_changed $$tmpdir/status $builddir/$ref_control/status;
        overwrite_if_changed $$tmpdir/available $builddir/$ref_control/available;
        rm -rf "$$tmpdir";
    """,
    restat=True,
    output_type=(str, str, OstreeRef),
    outputs=[
        '$builddir/$ref_control/status',
        '$builddir/$ref_control/available',
        '$ostree_repo/refs/heads/$ref_info'],
    order_only=["$ostree_repo/config"],
    inputs=["$ostree_repo/refs/heads/$ref_control",
            "$ostree_repo/refs/heads/$ref_data"])
...
class Apt(object):
        ...
        try:
            with self.ninja.open(lockfile) as f:
                for pkg in parse_packages(f):
                    ...
                    data, control = self.fix_package(pkg, data, control)
                    all_data.append(data.filename)
                    head, sep, tail = control.ref.rpartition('/')
                    ref_info = f'{head}/info'
                    status, available, info = make_dpkg_info.build(
                        self.ninja, sha256sum=pkg['SHA256'],
                        pkgname=pkg['Package'], ref_data=data.ref, ref_control=control.ref, ref_info=ref_info)
    ...
    def fix_package(self, pkg, data, control):
        """
